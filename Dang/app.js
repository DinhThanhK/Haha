...
// Fix 5: đếm số người theo ngày đang lọc
function updateExportCount() {
  ...
  if (!badge) return;
  let count = STATE.attendanceList.length;
  if (filterDate) {
    const [y, m, d] = filterDate.split('-');
    const viDate = `${d}/${m}/${y}`;
    count = STATE.attendanceList.filter(r => r.date === viDate).length;
  }
  badge.textContent = count + ' người';
}
...
function setSliderPct(v) {
  ...
  // Fix 3: bù offset thumb (11px = half of 22px thumb) so gradient aligns with center of thumb
  const thumbHalf = 11;
...
function syncRadiusFromSlider(val, el) {
  ...
  // Fix 3: dùng el.offsetWidth thực để tính pct chính xác
  if (el) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Fix 5: Đặt mặc định ngày hôm nay cho bộ lọc xuất
