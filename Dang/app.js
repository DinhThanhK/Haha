...
const STATE = {
  ...
  // Khởi tạo gốc, nhưng nó sẽ bị ghi đè ngay bởi Firebase nếu Admin đã "Phát điểm"
...
function renderLeafletMap(userLat, userLng, dist) {
  ...
  // Chỉ vẽ vị trí user khi có tọa độ thực (không fallback sang tọa độ mặc định)
...
function resetForm() {
  ...
  // Chỉ xóa token, giữ lại thông tin nếu đang bật ghi nhớ
...
function initAdminMap() {
  ...
  // Cập nhật circle realtime khi thay đổi bán kính
  document.getElementById('session-radius').addEventListener('input', function() {
    if (adminRadiusCircle) {
      adminRadiusCircle.setRadius(parseInt(this.value) || 300);
    }
  });
}
